...
def user_playlists(username):
    ...
    try:
        ...
        ###############################################################################
        sql = """SELECT
            mc.collection_id, mc.collection_name, mcc.media_id
        FROM
            mediaserver.MediaCollection mc natural join mediaserver.MediaCollectionContents mcc
            on (mc.collection_id=mcc.collection_id)
        WHERE
            mc.username=%s
...
def user_podcast_subscriptions(username):
    ...
    try:
        ...
        sql = """SELECT
            sp.podcast_id, mpd.podcast_title
        FROM
            mediaserver.Subscribed_Podcasts sp left outer join
            (mediaserver.Podcast natural join mediaserver.PodcastMetaData) mpd
            on (sp.podcast_id=mpd.podcast_id)
        WHERE
            sp.username=%s
...
def user_in_progress_items(username):
    ...
    try:
        ...
        sql = """SELECT *
        FROM
            mediaserver.UserMediaConsumption umc left outer join mediaserver.MediaItemMetaData mmid
            on (umc.media_id=mmid.media_id)
        WHERE
            umc.progress < 100 AND umc.username=%s
        order by umc.media_id;"""
...
def get_alltvshows():
    ...
    try:
        ...
        #############################################################################
        sql = """SELECT s.tvshow_id, s.tvshow_title, count(*) as count
        FROM mediaserver.tvshow s left outer join mediaserver.tvepisode sep on (s.tvshow_id = sep.tvshow_id)
...
def get_song(song_id):
    ...
    try:
        ...
        # (mediaserver.mediaitemmetadata natural join mediaserver.metadata natural join mediaserver.MetaDataType natural join mediaserver.song_artists natural join mediaserver.artist) mmd
        # on (s.song_id=mmd.media_id)
        # where s.song_id=%s;
        # """

        sql = '''
        select song_title, artist_name artists, s.length length
        from mediaserver.song s join
...
def get_song_metadata(song_id):
    ...
    try:
        ...
        sql = """
        select song_id, md_type_name, md_value, md_id
        ...
        select song_id, md_type_name, md_value, md_id
        from mediaserver.song s natural join (mediaserver.song_artists sa join mediaserver.artist a
            on sa.performing_artist_id = a.artist_id natural join mediaserver.artistmetadata natural join mediaserver.MetaDataType natural join mediaserver.metadata)
        where s.song_id=%s;
        """
...
def get_podcast(podcast_id):
    ...
    try:
        ...
        FROM mediaserver.podcast p left outer join
            (mediaserver.podcastepisode pe
            natural join mediaserver.PodcastMetaData
            natural join mediaserver.MetaData
            natural join mediaserver.MetaDataType) pnew
            on (p.podcast_id=pnew.podcast_id)
...
def get_all_podcasteps_for_podcast(podcast_id):
    ...
    try:
        ...
        (mediaserver.podcastepisode natural join mediaserver.PodcastMetaData natural join mediaserver.MetaData natural join mediaserver.MetaDataType) pemd
        on (pe.podcast_id=pemd.podcast_id)
...
#####################################################
#   Add podcast ep to deleted table
#####################################################
def delete_podcastep(podcast_id, media_id):
    ...
    try:
        # Try executing the SQL and get from the database
        sql = """INSERT INTO mediaserver.DeletedPodcastepisode (media_id, podcast_episode_length, podcast_episode_published_date, podcast_episode_title, podcast_episode_uri, podcast_id)
                SELECT media_id, podcast_episode_length, podcast_episode_published_date, podcast_episode_title, podcast_episode_uri, podcast_id FROM mediaserver.PodcastEpisode
                WHERE podcast_id = %s, media_id = %s;
                DELETE FROM mediaserver.PodcastEpisode
                WHERE podcast_id = %s, media_id = %s;"""
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error getting Albums with ID: "+media_id, sys.exc_info()[0])
        raise
    cur.close()                     # Close the cursor
    conn.close()                    # Close the connection to the db
    return None
...
#####################################################
#   Undo podcast ep deletion
#####################################################
def undo_delete_podcastep(podcast_id, media_id):
    ...
    try:
        # Try executing the SQL and get from the database
        sql = """INSERT INTO mediaserver.PodcastEpisode (media_id, podcast_episode_length, podcast_episode_published_date, podcast_episode_title, podcast_episode_uri, podcast_id)
                SELECT media_id, podcast_episode_length, podcast_episode_published_date, podcast_episode_title, podcast_episode_uri, podcast_id FROM mediaserver.DeletedPodcastepisode
                WHERE podcast_id = %s, media_id = %s;
                DELETE FROM mediaserver.DeletedPodcastepisode
                WHERE podcast_id = %s, media_id = %s;"""
        r = dictfetchall(cur,sql,(media_id,))
        print("return val is:")
        print(r)
...
def get_deleted_podcastep():
    ...
    try:
        # Try executing the SQL and get from the database
        sql = """SELECT * FROM mediaserver.DeletedPodcastEpisode;"""
        r = dictfetchall(cur,sql,(media_id,))
        print("return val is:")
        print(r)
        cur.close()                     # Close the cursor
...
#####################################################
#   Add tvshow ep to deleted table
#####################################################
def delete_tvepisode(media_id):
    ...
    try:
        # Try executing the SQL and get from the database
        sql = """INSERT INTO mediaserver.DeletedTVEpisode (media_id, tvshow_id, tvshow_episode_title, season, episode, air_date)
                SELECT media_id, tvshow_id, tvshow_episode_title, season, episode, air_date FROM mediaserver.TVEpisode
                WHERE media_id = %s;
                DELETE FROM mediaserver.TVEpisode
                WHERE media_id = %s;"""
        r = dictfetchall(cur,sql,(media_id,))
        print("return val is:")
        print(r)
...
#####################################################
#   Undo tvshow ep deletion
#####################################################
def undo_delete_tvepisode(media_id):
    ...
    try:
        # Try executing the SQL and get from the database
        sql = """INSERT INTO mediaserver.TVEpisode (media_id, tvshow_id, tvshow_episode_title, season, episode, air_date)
                SELECT media_id, tvshow_id, tvshow_episode_title, season, episode, air_date FROM mediaserver.DeletedTVEpisode
                WHERE media_id = %s;
                DELETE FROM mediaserver.DeletedTVEpisode
                WHERE media_id = %s;"""
        r = dictfetchall(cur,sql,(media_id,))
        print("return val is:")
        print(r)
...
def get_deleted_tvepisodes():
    ...
    try:
        # Try executing the SQL and get from the database
        sql = """SELECT * FROM mediaserver.DeletedTVEpisode;"""
        r = dictfetchall(cur,sql)
        print("return val is:")
        print(r)
        cur.close()                     # Close the cursor
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error getting Albums with ID: ", sys.exc_info()[0])
        raise
    cur.close()                     # Close the cursor
    conn.close()                    # Close the connection to the db
    return None
...
def get_album(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT album_title, md_type_name, md_value FROM mediaserver.album LEFT OUTER JOIN mediaserver.albummetadata USING(album_id) LEFT OUTER JOIN mediaserver.metadata USING(md_id) LEFT OUTER JOIN mediaserver.metadatatype USING(md_type_id)
            WHERE album_id = %s;
        """
...
def get_album_songs(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT song_id, song_title, artist_name, album_id FROM mediaserver.song LEFT OUTER JOIN mediaserver.song_artists USING(song_id) INNER JOIN mediaserver.artist ON(performing_artist_id = artist_id) LEFT OUTER JOIN mediaserver.album_songs USING(song_id)
            WHERE album_id = %s;
        """
...
def get_album_genres(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT DISTINCT md_value FROM mediaserver.song LEFT OUTER JOIN mediaserver.album_songs USING(song_id) 
        LEFT OUTER JOIN mediaserver.mediaitem ON(song_id = media_id) 
        LEFT OUTER JOIN mediaserver.mediaitemmetadata USING(media_id) 
        LEFT OUTER JOIN mediaserver.metadata USING(md_id) NATURAL JOIN mediaserver.metadatatype 
        WHERE album_id = %s;
        """
...
def get_genre_songs(genre_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT song_id as item_id, song_title as title, 'song' as type
        FROM ((mediaserver.song JOIN mediaserver.MediaItemMetaData ON (song_id = media_id))
        JOIN mediaserver.MetaData USING (md_id)) JOIN mediaserver.MetaDataType USING (md_type_id)
...
def get_genre_podcasts(genre_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT podcast_id as item_id, podcast_title as title, 'podcast' as type
...
def get_genre_movies_and_shows(genre_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        (SELECT tvshow_id as item_id, tvshow_title as title, 'tv show' as type
        FROM ((mediaserver.TVShow JOIN mediaserver.TVShowMetaData USING (tvshow_id)) 
        JOIN mediaserver.MetaData USING (md_id)) JOIN mediaserver.MetaDataType USING (md_type_id)
        WHERE md_type_name = 'film genre' AND md_id = %s
        GROUP BY tvshow_id, tvshow_title)
        UNION
        (SELECT movie_id as item_id, movie_title as title, 'movie' as type
...
def get_tvshow(tvshow_id):
    ...
    try:
        ...
        from mediaserver.tvshow ts left outer join
            (mediaserver.tvshowmetadata natural join mediaserver.metadata natural join mediaserver.MetaDataType) tsmd
        on (ts.tvshow_id=tsmd.tvshow_id)
...
def get_deleted_movie():
    ...
    try:
        # Try executing the SQL and get from the database
        sql = """SELECT * FROM mediaserver.DeletedMovie;"""
        r = dictfetchall(cur,sql,(movie_id,))
        print("return val is:")
        print(r)
        cur.close()                     # Close the cursor
...
#   Add a new Song
#####################################################
def add_song_to_db(storage_location, song_description, song_title, length, song_genre, artist_name, artwork):
    """
    Get all the matching Movies in your media server
    """
    #########
    ...
    try:
        ...
        cur.execute(sql, (storage_location, song_description, song_title, length, song_genre, artist_name, artwork))
        conn.commit()  # Commit the transaction
        r = cur.fetchone()
        print("return val is:")
        print(r)
