...
def single_podcast(podcast_id):
    ...
    #5.2.6 f implementation required for last functionality
...
def single_podcastep(media_id):
    ...
    # Get a list of all podcast eps by media_id from the database
    podcastep = None
...
def single_genre(genre_id):
    ...
    page['title'] = 'GENRE SEARCH' # Add the title

    single_genre = []

    type = (database.get_genre_type(genre_id)[0])['md_type_name']
    if type == 'film genre':
        single_genre = database.get_genre_movies_and_shows(genre_id)
    elif type == 'podcast genre':
        single_genre = database.get_genre_podcasts(genre_id)
    elif type == 'song genre':
        single_genre = database.get_genre_songs(genre_id)

    if single_genre == None or single_genre == []:
        single_genre = []
        page['bar'] = False
        flash("No matching tv shows found, please try again")
    else:
        page['bar'] = True
        flash('Found '+str(len(single_genre))+' results!')
        session['logged_in'] = True
...
def add_song():
    ...
    if request.method == 'POST':
        ...
        if ('location' not in request.form):
            newdict['location'] = 'Empty location'
        else:
        ...
        if ('song_title' not in request.form):
            ...
            newdict['title'] = request.form['song_title']
        ...
        #forward to the database to manage insert
        song = database.add_song_to_db(newdict['location'],newdict['description'],newdict['title'],newdict['song_length'],newdict['song_genre'],newdict['artist'],newdict['artwork'])
...
#####################################################
#   New functionality
#   SuperUser - Delete access
#####################################################

# TO-DO: Colby
def delete_song(song_id):
    """
    Delete a single song by song_id in your media server
    ...
    # Check if the user is a SuperUser, if not: do nothing
    elif not (database.is_superuser(request.form['username'])):
        return None

    page['title'] = 'Deleting Song ID: ' + song_id

    # Get a song by its song_id from the database
    song = None
    song = database.get_song(song_id)
    ...
    # Connect to database and delete song
    ...
    try:
        ...
        sql += """insert into mediaserver.deletedsong (song_id, song_title, length)
        select song_id, song_title, length
        from mediaserver.song s left outer join
            (mediaserver.songmetadata natural join mediaserver.metadata natural join mediaserver.MetaDataType) smd
        on (s.song_id=smd.song_id)
        where s.song_id=%s;"""
        ...
        print("Unexpected error getting Song with ID: '" + song_id + "'", sys.exc_info()[0])
        raise
    cur.close()                     # Close the cursor
    conn.close()                    # Close the connection to the db
    return None

def undo_delete_song(song_id):
    """
    Undo the deletion of a single song by song_id in your media server
    User must be a SuperUser
    """
    # Check if the user is logged in, if not: back to login.
    if('logged_in' not in session or not session['logged_in']):
    ...
    page['title'] = 'Restoring Song ID: ' + song_id
    ...
    # Connect to database and restore deleted song
    conn = database_connect()
    if(conn is None):
        return None
    cur = conn.cursor()
    try:
        ...
        # add back to songs table
        sql += """insert into mediaserver.song (song_id, song_title, length)
        select song_id, song_title, length
        from mediaserver.deleted s left outer join
            (mediaserver.songmetadata natural join mediaserver.metadata natural join mediaserver.MetaDataType) smd
        on (s.song_id=smd.song_id)
        where s.song_id=%s;"""
...
def get_delete_movie():
    ...
    single_movie = database.get_deleted_movie()

    if single_movie == None or single_movie == []:
        single_movie = []
        page['bar'] = False
        flash("No matching movie found, please try again")

    return render_template('listdeleteditems/listdeletedmovies.html',
                           session=session,
                           page=page,
                           user=user_details,
                           single_movie=movie)
...
def delete_movie(movie_id):
    """
    Delete a single movie by movie_id in your media server
    User must be a SuperUser
    """
    # Check if the user is logged in, if not: back to login.
    if('logged_in' not in session or not session['logged_in']):
    ...
    page['title'] = 'Deleting Movie ID: ' + movie_id

    # Get a movie by its song_id from the database
    movie = None
    movie = database.get_movie(movie_id)
    ...
    try:
        ...
        # insert into deleted table
        sql += """insert into mediaserver.deletedmovie (movie_id, movie_title, release_year)
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error getting Movie with ID: '" + movie_id + "'", sys.exc_info()[0])
        raise
    cur.close()                     # Close the cursor
    conn.close()                    # Close the connection to the db
    return None
...
def undo_delete_movie(movie_id):
    """
    Undo the deletion of a single movie by movie_id in your media server
    User must be a SuperUser
    """
    # Check if the user is logged in, if not: back to login.
    if('logged_in' not in session or not session['logged_in']):
    ...
    page['title'] = 'Restoring Movie ID: ' + movie_id

    # Get a movie by its movie_id from the database
    movie = None
    movie = database.get_movie(movie_id)
    ...
    # Connect to database and restore deleted movie
    conn = database_connect()
    if(conn is None):
        return None
    cur = conn.cursor()
    try:
        ...
        # add back to movie table
        sql += """insert into mediaserver.movie (movie_id, movie_title, release_year)
        select movie_id, movie_title, release_year
        from mediaserver.deletedmovie m left outer join
            (mediaserver.mediaitemmetadata natural join mediaserver.metadata natural join mediaserver.MetaDataType) mmd
        on (m.movie_id=mmd.movie_id)
        where m.movie_id=%s;"""
...
# TO-DO: Yupeng
def get_delete_podcastep():
    if ('logged_in' not in session or not session['logged_in']):
        return redirect(url_for('login'))

    single_podcastep = database.get_deleted_podcastep()

    if single_podcastep == None or single_podcastep == []:
        single_podcastep = []
        page['bar'] = False
        flash("No matching podcast episode found, please try again")

    return render_template('listdeleteditems/listdeletedpodcasteps.html',
                           session=session,
                           page=page,
                           user=user_details,
                           podcastep=podcastep)
...
def undo_delete_podcastep(podcast_id, media_id):
    database.undo_delete_podcastep(podcast_id, media_id)

def delete_podcastep(podcast_id, media_id):
    database.delete_podcastep(podcast_id, media_id)

def delete_tvshowep(media_id):
    database.delete_tvepisode(media_id)
    pass

def undo_delete_tvshowep(media_id):
    database.undo_delete_tvepisode(media_id)
    pass



@app.route('/list/deletedtvepisodes')
def get_deleted_tvshowep():

    if('logged_in' not in session or not session['logged_in']):
        return redirect(url_for('login'))

    alltvshows = None
    alltvshows = database.get_deleted_tvepisodes()
    ...
    return render_template('listdeleteditems/listdeletedtvshoweps.html',
                           session=session,
                           page=page,
                           user=user_details,
                           alltvshows = alltvshows)
